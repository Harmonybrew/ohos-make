...
```sh
cd ~
curl -fLO https://github.com/Harmonybrew/ohos-make/releases/download/4.4.1/make-4.4.1-ohos-arm64.tar.gz
tar -zxf make-4.4.1-ohos-arm64.tar.gz
export PATH=~/make-4.4.1-ohos-arm64/bin:$PATH
...
示例：
```sh
hdc file send make-4.4.1-ohos-arm64.tar.gz /data
hdc shell

cd /data
tar -zxf make-4.4.1-ohos-arm64.tar.gz
export PATH=/data/make-4.4.1-ohos-arm64/bin:$PATH
...
git clone https://github.com/Harmonybrew/ohos-make.git
cd ohos-make
./build.sh
```
